...
impl OTCryptoLibHMAC<'_> {
    ...
    fn with_hmac_context<R, F>(&self, f: F) -> R
        where F: FnOnce(
            &mut otcrypto_mac_ef_bindings::hmac_context_t,
        ) -> R
    {
    ...
    fn add_data_int(
        ...
        let res = self.with_hmac_context(|hmac_context| {
            ...
            unsafe { otcrypto_mac_ef_bindings::otcrypto_hmac_update(
...
impl DeferredCallClient for OTCryptoLibHMAC<'_> {
    ...
    fn handle_deferred_call(&self) {
        match (self.data_slice.take(), self.data_slice_mut.take(), self.digest_buf.take()) {
            (Some(data_slice), None, None) => /* data slice */ {
                self.client.map(move |c| c.add_data_done(Ok(()), data_slice));
            },

            (None, Some(data_slice_mut), None) => /* data slice mut */ {
                self.client.map(move |c| c.add_mut_data_done(Ok(()), data_slice_mut));
            },

            (None, None, Some(digest_buf)) => /* hash done */ {
...
impl<'a> digest::DigestHash<'a, { SHA_256_OUTPUT_LEN_BYTES }> for OTCryptoLibHMAC<'a> {
    ...
    fn run(&'a self, digest: &'static mut [u8; 32]) -> Result<(), (ErrorCode, &'static mut [u8; 32])> {
        self.with_hmac_context(|hmac_context| {
            ...
            unsafe { otcrypto_mac_ef_bindings::otcrypto_hmac_final(
                hmac_context as *mut _,
...
impl<'a> digest::DigestVerify<'a,{ SHA_256_OUTPUT_LEN_BYTES  }> for OTCryptoLibHMAC<'a> {
    fn set_verify_client(&'a self, client: &'a dyn digest::ClientVerify<{ SHA_256_OUTPUT_LEN_BYTES }>){
        // see comment for dataclient
        unimplemented!()
    }
    fn verify(
...
impl<'a> digest::HmacSha256 for OTCryptoLibHMAC<'a> {
    fn set_mode_hmacsha256(&self, key: &[u8]) -> Result<(), ErrorCode> {
        ...
        self.with_hmac_context(|hmac_context| {
            ...
            let keyblob_words = unsafe { otcrypto_mac_ef_bindings::keyblob_num_words(key_config_rust) };
            ...
            unsafe { otcrypto_mac_ef_bindings::keyblob_from_key_and_mask(
            ...
            let checksum = unsafe { otcrypto_mac_ef_bindings::integrity_blinded_checksum(&blinded_key as *const _ as *const _) };

            blinded_key.checksum = checksum;
            //debug!("Blinded checksummed key: {:?}", &*blinded_key.validate(access).unwrap());


            let res = unsafe { otcrypto_mac_ef_bindings::otcrypto_hmac_init(
                hmac_context as *mut _,
